...
#     def causal_mask(self, size: int) -> torch.Tensor:
...
#         # Apply causal mask
#         mask = self.causal_mask(attention_scores.size(-1)).to(attention_scores.device)
...
class AttentionBlock(nn.Module):
    ...
    def __init__(
            ...
            rescale_output_factor: float = 1.0,
            eps: float = 1e-5,
            window_size: int = 32,  # Add a window size parameter for local attention
    ):
        super().__init__()
        self.channels = channels
    ...
    def causal_local_mask(self, size: int) -> torch.Tensor:
        mask = torch.triu(torch.ones(size, size), diagonal=1)
        local_mask = torch.tril(torch.ones(size, size), diagonal=self.window_size - 1)  # create a lower triangular mask for local attention
        combined_mask = mask + 1 - local_mask  # combine the masks
        return combined_mask.masked_fill(combined_mask == 1, float('-inf'))
    ...
    def forward(self, hidden_states):
        ...
        mask = self.causal_local_mask(attention_scores.size(-1)).to(attention_scores.device)
        attention_scores = attention_scores + mask.unsqueeze(0).unsqueeze(1) 
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # Add non-linearity after softmax with GELU activation
        attention_probs = F.gelu(attention_probs)
        ...
        # Add non-linearity after linear transformation with GELU activation
        hidden_states = F.gelu(self.proj_attn(hidden_states))
        hidden_states = hidden_states.transpose(-1, -2).reshape(batch, channel, height, width)
